...
const RadioButton = (props) => {
    ...
    return React.createElement("div", { "data-testid": 'radio' }, position == 'horizontal' ?
        React.createElement(Horizontal, { name: name, options: options, getValue: getValue, getError: getError, isSubmiting: isSubmiting, label: label, checked: checked, disabled: disabled, required: required }) :
        React.createElement(Vertical, { name: name, options: options, getValue: getValue, getError: getError, isSubmiting: isSubmiting, label: label, checked: checked, disabled: disabled, required: required }));
};
const Horizontal = ({ options, label, name, checked, disabled, getValue, getError, isSubmiting, required }) => {
...
const Vertical = ({ options, label, name, checked, disabled, getValue, getError, isSubmiting, required }) => {
    const [selected, setSelected] = useState(checked);
    const [hasError, setError] = useState(false);
    useEffect(() => {
        const pairValue = {};
